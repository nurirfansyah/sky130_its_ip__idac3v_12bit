v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
N -170 -150 -170 -120 {
lab=#net1}
N -90 -130 -90 -120 {
lab=#net2}
N 480 10 480 30 {
lab=GND}
N -170 -60 -170 30 {
lab=GND}
N -90 -60 -90 30 {
lab=GND}
N 90 -10 120 -10 {
lab=d3}
N 480 -60 480 -50 {
lab=#net3}
N -170 30 40 30 {
lab=GND}
N -90 -130 120 -130 {
lab=#net2}
N -170 -150 120 -150 {
lab=#net1}
N 50 -70 120 -70 {
lab=d0}
N 50 -50 120 -50 {
lab=d1}
N 50 -30 120 -30 {
lab=d2}
N -20 -90 -20 -70 {
lab=#net4}
N -20 -10 -20 30 {
lab=GND}
N -370 30 -170 30 {
lab=GND}
N -610 -20 -610 -10 {
lab=GND}
N -640 -130 -610 -130 {
lab=GND}
N -640 -130 -640 -10 {
lab=GND}
N -640 -10 -610 -10 {
lab=GND}
N -640 -240 -640 -130 {
lab=GND}
N -640 -240 -610 -240 {
lab=GND}
N -610 -100 -610 -80 {
lab=d5}
N -610 -100 -560 -100 {
lab=d5}
N -610 -210 -610 -190 {
lab=d4}
N -610 -210 -560 -210 {
lab=d4}
N -610 -320 -610 -300 {
lab=d3}
N -610 -320 -560 -320 {
lab=d3}
N 420 -150 540 -150 {
lab=out}
N 630 10 630 30 {
lab=GND}
N 630 -70 630 -50 {
lab=#net5}
N 630 -150 630 -130 {
lab=out}
N 420 -110 480 -110 {
lab=#net3}
N 480 -110 480 -60 {
lab=#net3}
N 540 -150 630 -150 {
lab=out}
N 420 -130 550 -130 {
lab=outn}
N 570 -130 570 -120 {
lab=outn}
N 550 -130 570 -130 {
lab=outn}
N 570 -60 570 -50 {
lab=#net6}
N 570 10 570 30 {
lab=GND}
N 480 30 570 30 {
lab=GND}
N 570 30 630 30 {
lab=GND}
N -610 -360 -610 -350 {
lab=GND}
N -640 -470 -610 -470 {
lab=GND}
N -640 -470 -640 -350 {
lab=GND}
N -640 -350 -610 -350 {
lab=GND}
N -640 -580 -640 -470 {
lab=GND}
N -640 -580 -610 -580 {
lab=GND}
N -610 -440 -610 -420 {
lab=d2}
N -610 -440 -560 -440 {
lab=d2}
N -610 -550 -610 -530 {
lab=d1}
N -610 -550 -560 -550 {
lab=d1}
N -610 -660 -610 -640 {
lab=d0}
N -610 -660 -560 -660 {
lab=d0}
N -640 -350 -640 -240 {
lab=GND}
N 40 30 40 130 {
lab=GND}
N 480 30 480 120 {
lab=GND}
N 90 10 120 10 {
lab=d4}
N 90 30 120 30 {
lab=d5}
N -380 -10 -380 0 {
lab=GND}
N -410 -120 -380 -120 {
lab=GND}
N -410 -120 -410 0 {
lab=GND}
N -410 0 -380 0 {
lab=GND}
N -410 -230 -410 -120 {
lab=GND}
N -410 -230 -380 -230 {
lab=GND}
N -380 -90 -380 -70 {
lab=d11}
N -380 -90 -330 -90 {
lab=d11}
N -380 -200 -380 -180 {
lab=d10}
N -380 -200 -330 -200 {
lab=d10}
N -380 -310 -380 -290 {
lab=d9}
N -380 -310 -330 -310 {
lab=d9}
N -380 -350 -380 -340 {
lab=GND}
N -410 -460 -380 -460 {
lab=GND}
N -410 -460 -410 -340 {
lab=GND}
N -410 -340 -380 -340 {
lab=GND}
N -410 -570 -410 -460 {
lab=GND}
N -410 -570 -380 -570 {
lab=GND}
N -380 -430 -380 -410 {
lab=d8}
N -380 -430 -330 -430 {
lab=d8}
N -380 -540 -380 -520 {
lab=d7}
N -380 -540 -330 -540 {
lab=d7}
N -380 -650 -380 -630 {
lab=d6}
N -380 -650 -330 -650 {
lab=d6}
N -410 -340 -410 -230 {
lab=GND}
N -410 -0 -410 30 {
lab=GND}
N -410 30 -370 30 {
lab=GND}
N -640 30 -410 30 {
lab=GND}
N -640 -10 -640 30 {
lab=GND}
N 40 130 40 230 {
lab=GND}
N 480 120 480 200 {
lab=GND}
N 40 200 480 200 {
lab=GND}
N 90 50 120 50 {
lab=d6}
N 90 70 120 70 {
lab=d7}
N 90 90 120 90 {
lab=d8}
N 90 110 120 110 {
lab=d9}
N 90 130 120 130 {
lab=d10}
N 90 150 120 150 {
lab=d11}
N -400 70 -400 130 {
lab=clk}
N -400 70 -300 70 {
lab=clk}
N 90 170 120 170 {
lab=clk}
N -400 190 -400 210 {
lab=GND}
N -400 210 40 210 {
lab=GND}
N -20 -90 120 -90 {
lab=#net4}
N 100 -110 120 -110 {
lab=GND}
N 100 -110 100 200 {
lab=GND}
C {devices/vsource.sym} -170 -90 0 0 {name=V1 value=3.3 savecurrent=false}
C {devices/vsource.sym} -90 -90 0 0 {name=V2 value=1.8 savecurrent=false}
C {devices/gnd.sym} 40 230 0 0 {name=l1 lab=GND}
C {devices/isource.sym} 480 -20 0 0 {name=I0 value=10u}
C {devices/vsource.sym} -610 -270 0 0 {name=V3 value="PULSE(1.8 0 50n 5n 5n 1600n 3200n)"}
C {devices/vsource.sym} -610 -160 0 0 {name=V4 value="PULSE(1.8 0 50n 5n 5n 3200n 6400n)"}
C {devices/vsource.sym} -610 -50 0 0 {name=V5 value="PULSE(1.8 0 50n 5n 5n 6400n 12800n)"}
C {devices/vsource.sym} -20 -40 0 0 {name=V6 value=0 savecurrent=false}
C {devices/lab_pin.sym} 50 -70 0 0 {name=p1 sig_type=std_logic lab=d0}
C {devices/lab_pin.sym} 50 -50 0 0 {name=p2 sig_type=std_logic lab=d1}
C {devices/lab_pin.sym} 50 -30 0 0 {name=p3 sig_type=std_logic lab=d2}
C {devices/lab_pin.sym} -560 -320 0 1 {name=p4 sig_type=std_logic lab=d3}
C {devices/lab_pin.sym} -560 -210 0 1 {name=p5 sig_type=std_logic lab=d4}
C {devices/lab_pin.sym} -560 -100 0 1 {name=p6 sig_type=std_logic lab=d5}
C {devices/vsource.sym} 630 -100 0 0 {name=V7 value=0}
C {devices/code.sym} -130 -570 0 0 {name=TT_MODELS1
only_toplevel=true
format="tcleval( @value )"
value=".lib \\\\$::SKYWATER_MODELS\\\\/sky130.lib.spice tt
.include \\\\$::SKYWATER_STDCELLS/../../sky130_fd_sc_hvl/spice/sky130_fd_sc_hvl.spice

.param mc_mm_switch=0
.param mc_pr_switch=1
"
}
C {devices/code.sym} 90 -570 0 0 {name=spice1 only_toplevel=false value="
.option wnflag=0
.save v(out) i(V7)
.control
tran 500n 3.2u
plot v(out)
plot i(V7)
write idac2.raw v(out)
.endc
"}
C {devices/lab_wire.sym} 540 -150 0 0 {name=p7 sig_type=std_logic lab=out}
C {devices/res.sym} 630 -20 0 0 {name=R1
value=50
footprint=1206
device=resistor
m=1}
C {devices/vsource.sym} 570 -90 0 0 {name=V8 value=0}
C {devices/res.sym} 570 -20 0 0 {name=R2
value=50
footprint=1206
device=resistor
m=1}
C {devices/lab_wire.sym} 510 -130 0 0 {name=p8 sig_type=std_logic lab=outn}
C {devices/vsource.sym} -610 -610 0 0 {name=V9 value="PULSE(1.8 0 50n 5n 5n 200n 400n)"}
C {devices/vsource.sym} -610 -500 0 0 {name=V10 value="PULSE(1.8 0 50n 5n 5n 400n 800n)"}
C {devices/vsource.sym} -610 -390 0 0 {name=V11 value="PULSE(1.8 0 50n 5n 5n 800n 1600n)"}
C {devices/lab_pin.sym} -560 -660 0 1 {name=p9 sig_type=std_logic lab=d0}
C {devices/lab_pin.sym} -560 -550 0 1 {name=p10 sig_type=std_logic lab=d1}
C {devices/lab_pin.sym} -560 -440 0 1 {name=p11 sig_type=std_logic lab=d2}
C {devices/lab_pin.sym} 90 -10 0 0 {name=p12 sig_type=std_logic lab=d3}
C {devices/lab_pin.sym} 90 10 0 0 {name=p13 sig_type=std_logic lab=d4}
C {devices/lab_pin.sym} 90 30 0 0 {name=p14 sig_type=std_logic lab=d5}
C {idac12bitsegmented.sym} 270 10 0 0 {name=x2}
C {devices/vsource.sym} -380 -260 0 0 {name=V12 value="PULSE(1.8 0 50n 5n 5n 102.4u 204.8u)"}
C {devices/vsource.sym} -380 -150 0 0 {name=V13 value="PULSE(1.8 0 50n 5n 5n 204.8u 409.6u)"}
C {devices/vsource.sym} -380 -40 0 0 {name=V14 value="PULSE(0 1.8 50n 5n 5n 409.6uu 819.2u)"}
C {devices/lab_pin.sym} -330 -310 0 1 {name=p15 sig_type=std_logic lab=d9}
C {devices/lab_pin.sym} -330 -200 0 1 {name=p16 sig_type=std_logic lab=d10}
C {devices/lab_pin.sym} -330 -90 0 1 {name=p17 sig_type=std_logic lab=d11}
C {devices/vsource.sym} -380 -600 0 0 {name=V15 value="PULSE(1.8 0 50n 5n 5n 12.8u 25.6u)"}
C {devices/vsource.sym} -380 -490 0 0 {name=V16 value="PULSE(1.8 0 50n 5n 5n 25.6u 51.2u)"}
C {devices/vsource.sym} -380 -380 0 0 {name=V17 value="PULSE(1.8 0 50n 5n 5n 51.2u 102.4u)"}
C {devices/lab_pin.sym} -330 -650 0 1 {name=p18 sig_type=std_logic lab=d6}
C {devices/lab_pin.sym} -330 -540 0 1 {name=p19 sig_type=std_logic lab=d7}
C {devices/lab_pin.sym} -330 -430 0 1 {name=p20 sig_type=std_logic lab=d8}
C {devices/lab_pin.sym} 90 50 0 0 {name=p21 sig_type=std_logic lab=d6}
C {devices/lab_pin.sym} 90 70 0 0 {name=p22 sig_type=std_logic lab=d7}
C {devices/lab_pin.sym} 90 90 0 0 {name=p23 sig_type=std_logic lab=d8}
C {devices/lab_pin.sym} 90 110 0 0 {name=p24 sig_type=std_logic lab=d9}
C {devices/lab_pin.sym} 90 130 0 0 {name=p25 sig_type=std_logic lab=d10}
C {devices/lab_pin.sym} 90 150 0 0 {name=p26 sig_type=std_logic lab=d11}
C {devices/vsource.sym} -400 160 0 0 {name=V18 value="PULSE(0 3.3 50n 5n 5n 100n 200n)"}
C {devices/lab_pin.sym} -300 70 0 1 {name=p27 sig_type=std_logic lab=clk}
C {devices/lab_pin.sym} 90 170 0 0 {name=p28 sig_type=std_logic lab=clk}
